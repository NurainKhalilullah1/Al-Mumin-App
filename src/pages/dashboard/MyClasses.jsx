...
import { getStudentsByClass, saveAttendance, getAttendance } from '../../utils/db';

const MyClasses = () => {
  const [activeTab, setActiveTab] = useState('register'); // 'students' or 'register'
  const [currentClass, setCurrentClass] = useState('JSS 2');
  ...
  useEffect(() => {
    // 1. Load Students
    const classList = getStudentsByClass(currentClass);
    setStudents(classList);

    // 2. Load Existing Attendance for this date (if any)
    const existingRecord = getAttendance(currentClass, date);
    if (Object.keys(existingRecord).length > 0) {
      setAttendance(existingRecord);
    } else {
      // Default to empty or all present? Let's leave empty.
      setAttendance({});
    }
  }, [currentClass, date]);
  ...
  const handleSaveAttendance = () => {
    saveAttendance(currentClass, date, attendance);
  };
    ...
    <div className="animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row justify-between items-end mb-8">
        <div>
          ...
          <p className="text-gray-500 mt-1">Manage Class: <span className="font-bold text-gray-800">{currentClass}A</span></p>
        </div>
